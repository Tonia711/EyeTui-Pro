...
class ChatAskRequest(BaseModel):
    """Chat ask request payload."""
    question: str


class ChatSource(BaseModel):
    """Data source metadata for chatbot responses."""
    source_type: str
    source_value: str


class ChatAskResponse(BaseModel):
    """Chat ask response payload."""
    route: str
    intent: str
    answer: str
    data: dict
...
class ChatIntentPayload(BaseModel):
    """
    Strictly validated routing payload generated by LLM.
    """
    route: Literal["business_qa", "doc_qa", "out_of_scope"]
    intent: Optional[
        Literal[
            "inventory_overview",
    ...
    invoice_number: Optional[str] = None
    doc_search_terms: list[str] = []


class ChatQueryPlan(BaseModel):
    """
    Generic validated execution plan for business queries.
    """
    route: Literal["business_qa", "doc_qa", "out_of_scope"]
    entity: Optional[Literal["lens", "invoice", "supplier", "company", "site", "reconciliation"]] = None
    operation: Optional[Literal["overview", "count", "exists", "detail", "summary"]] = None
    serial_number: Optional[str] = None
    invoice_number: Optional[str] = None
